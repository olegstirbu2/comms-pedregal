...
export const CATEGORIES = ['All', 'Consumer', 'Merchant', 'Courier', 'General'] as const;
export const COUNTRIES = ['All', 'Finland', 'Sweden', 'Norway', 'Denmark', 'Iceland'] as const;
export const LANGUAGES = ['All', 'English'] as const;
...
export function filterTemplates(
  ...
  return templates.filter((template) => {
    ...
    // Country filter (skip if "All")
    if (filters.country && filters.country !== 'All' && template.country !== filters.country) {
      return false;
    }
    
    // Language filter (skip if "All" - all templates are English now)
    if (filters.language && filters.language !== 'All' && filters.language !== 'English') {
      return false;
    }
...
export type SortColumn = 'title' | 'category' | 'country' | 'language' | 'owner' | 'creationDate' | 'updateDate';
